...
class DatabaseWriter:
    ...
    def read_from_airports(self):
        with self.connection.cursor() as cursor:
            cursor.execute("SELECT abbreviation, id FROM airports")
        table = cursor.fetchall()
        return table

    def write_from_api(self, airports):
        with self.connection.cursor() as cursor:
            cursor.executemany("""INSERT INTO airports (abbreviation, name)
                                                 VALUES (%s, %s);""", [item for item in airports.items()])
    ...
    def write_from_scrape(self, trips):
        airports = self.read_from_airports()
        self.airports_table = {airport['abbreviation']: airport['id'] for airport in airports}
        ...
        for trip_unique_id, trip_details in trips.items():
            for flight_num_in_trip, flight_details in enumerate(zip(trip_details['Departure_airport'],
                ...
                departure_airport_id = self.airports_table.get(flight_details[0])
                arrival_airport_id = self.airports_table.get(flight_details[1])
